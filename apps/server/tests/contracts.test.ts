...
import { validateBudget, type Budget } from "../src/domain/budget.js";
...
describe("contract fixtures satisfy the domain invariants", () => {
  ...
  it("accepts the budget fixture", () => {
    const fixture = readFixture<Budget>("budget");
    expect(() => validateBudget(fixture)).not.toThrow();
    expect(validateContract("budget", fixture).valid).toBe(true);
  });
