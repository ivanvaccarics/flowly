...
- Sticky 64 px white top bar carrying the current section's `h1`, the vault state
  ...
  the session controls (Lock session, Lock all). A breadcrumb eyebrow above the
  `h1` names the family the section belongs to (Analysis & trend, Money &
...
- **Data table:** uppercase micro-headers over a hairline, 44 px rows, hover tint,
  ...
  under the row. An editable cell — payee, note, tags, amount — opens its row on
  a double-click with the caret already in that cell, next to the row's **Edit**
  button, which stays as the keyboard-friendly way in. The row's second line
  names where it came from (the bank's own description, or the source); a
  provider's raw row id lives only behind **Raw**.
