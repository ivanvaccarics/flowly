...
| Rules | Violet engine banner with the rule counts and the backfill action, condition builder (AND/OR, per-field operators, amount currency) that **creates or edits** a rule — **Edit** loads a tile back into the builder and save replaces its conditions while keeping its id, state and order — tag selection, and a side column of rule tiles with an on/off switch, the matched expression, edit and delete |
