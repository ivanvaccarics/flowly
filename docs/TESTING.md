...
| Fill the **New transaction** form above the ledger and press **Add transaction** | The movement is saved, the form empties itself and keeps its place, and the new row is at the top of the ledger (the **Reset** button empties it without writing) |
...
| Click the date (or the payee, the note, the tags, the amount) of a row | The movement opens in a dialog over the page, prefilled with its account, date, amount, payee, note, status and tags; **Cancel** closes it without writing, and the row's **Edit** button opens it too. The status chip is the exception: a click there only switches booked ↔ pending |
...
| Filter by account, date range, tag, status, source, transfer or amount, or by free text | The list narrows and the count updates; nothing is filtered in the browser only. The amount is written in the selected account's currency (EUR when no account is chosen) and an amount that is not a number is refused with a line above the list |
